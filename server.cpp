...
int main(int argc, char *argv[]) {
	...
	// listen()으로 서버소켓으로 오는 클라이언트 요청 대기 : 계속 listen() 중...
	...
	client_sock = accept(server_sock, (struct sockaddr*)&client_addr, &client_addr_size);	// 데이터를 받고...
	...
	char data[30];										
	int str_len = read(client_sock, data, sizeof(data) - 1);	// null 문자 제거
	if (str_len == -1)
	...
	//// 클라이언트로부터 데이터를 읽고, 데이터 내용과 길이를 저장함	// 서버 소켓 > 클라이언트 소켓으로 나뉨
	//message[0] = '\0';												// 날빌 : 맨 앞에 null를 넣으면 다 비워짐
	//int str_len = read(client_sock, message, sizeof(message) - 1);	// null 문자 제거
	//if (str_len == -1)
	//	error_handling("read() error");
	...
	// 소켓 닫음
	close(client_sock);
	close(server_sock);		// listen을 중간에 while문을 통해 살릴 수도 있음
