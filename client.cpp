...
// main의 매개변수 각각에 주목 argc : 매개변수 개수
int main(int argc, char *argv[]) {

	// 서버, 클라이언트 소켓 파일 디스크립터 변수 선언
	int sock;

	// sockaddr_in 구조체 변수 선언 : 리눅스에서 사용
	...
	if (sock == -1)
		error_handling("socket() error");		// error_handling _ 창환씨 직접 제작...

	// 서버 정보 초기화 및 정보 할당
	memset(&server_addr, 0, sizeof(server_addr));
	server_addr.sin_family = AF_INET;						// AF_INET - IPv4를 쓴다는 의미
	server_addr.sin_addr.s_addr = inet_addr(argv[1]);		// 연결할 서버 IP 설정. argv[1](매개변수 값)이 들어감 
	...
	char data2[30];			// 서버로부터 받은 데이터 저장
	str_len = read(sock, data2, sizeof(data2) - 1);
	if (str_len == -1)		// 데이터가 정상적으로 받지 못했을 때
